...
from .forms import mover_registration_form
from .models import mover_moverinfo, user_mover
...
class Registration(FormView):    # code for template is given below the view's code
    ...
    success_url = '/mover/home'
    form_class = mover_registration_form
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if form.is_valid():
                ...
                data = form.cleaned_data["email_m"]
            if self.validate_email_address(data) is True:
                '''
                If the input is an valid email address, then the following code will lookup for users associated with that email address. If found then an email will be sent to the address, else an error message will be printed on the screen.
                '''
                associated_mover = mover_moverinfo.objects.filter(Q(email=data))
                if associated_mover.exists():
                    result = self.form_valid(form)
                    messages.error(
                        request, 'Customer is already registered kindly login')
                    return result
                else:
                    mover = mover_moverinfo.objects.create(name=data_dict["name_m"],email=data_dict['email_m'],mobile=data_dict['mobile_m'])
                    user = User.objects.create_user(
                    username=form.cleaned_data['email_m'],
                    password=form.cleaned_data['password_m'],
                    email=form.cleaned_data['email_m']
                    )
                    user = authenticate(username=form.cleaned_data['email_m'], password=form.cleaned_data['password_m'])
                    if user is not None:
                       if user.is_active:
                            login(request, user)
                    save_relation(mover, user)
    ...
    if request.method == 'GET':
        if not request.user.is_authenticated():
            ...
            if user_type == 'DA':
                ...
                return HttpResponse(loader.get_template('mover.html').render(context,request))
...
def create_mover_profile(user):
    Profile.objects.create(user=user, user_type='DA')
    save_mover_profile(user)
